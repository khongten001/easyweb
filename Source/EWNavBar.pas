...
uses Types, SysUtils, VCL.Controls, Json, EWConst;
...
begin
  inherited;
  if AParams.Values[C_ACTION] = C_SEARCH then
  begin

    if Assigned(FOnSearch) then
      FOnSearch(Self, AParams.Values[C_VALUE]);
    Exit;
  end;

  AIndex := StrToIntDef(AParams.Values[C_INDEX], -1);
  ASubIndex := StrToIntDef(AParams.Values[C_SUB_INDEX], -1);
...
begin
  ...
  try
    AIndex := StrToIntDef(AJson.GetValue(C_INDEX).Value, -1);
    ASubIndex := -1;
    if AJson.GetValue(C_SUB_INDEX) <> nil then
      ASubIndex := StrToIntDef(AJson.GetValue(C_SUB_INDEX).Value, -1);
    if Assigned(FOnItemClick) then
...
begin
  ...
  if Assigned(FOnBrandClick) then
    AddObjectEvent(Name+'Brand', C_CLICK, [], AListners, '');

  if Assigned(FOnSearch) then
  begin
    AddObjectEvent(Name+'SearchButton', C_CLICK, [C_ACTION+'='+C_SEARCH], AListners, 'document.getElementById("'+Name+'SearchInput").value');
    AddObjectEvent(Name+'SearchInput', C_SEARCH, [C_ACTION+'='+C_SEARCH], AListners, 'document.getElementById("'+Name+'SearchInput").value');
  end;

  for AItem in FItems do
  begin
    // first add the nav item click...
    AddObjectEvent(Name+C_ITEM+AItem.Index.ToString, C_CLICK, [C_INDEX+'='+AItem.Index.ToString], AListners, '');

    // now any drop-down menu item clicks...
    for ICount := 0 to TEWNavBarItem(AItem).DropdownItems.Count-1 do
    begin
      AddObjectEvent(Name+C_ITEM+AItem.ID.ToString+'_'+C_SUB_ITEM+ICount.ToString,
                     C_CLICK,
                     [C_INDEX+'='+AItem.Index.ToString, C_SUB_INDEX+'='+ICount.ToString],
                     AListners, '');
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := Result + '<a id="'+FNavBar.Name+C_ITEM+ID.ToString+'_'+C_SUB_ITEM+ICount.ToString+'" class="dropdown-item" href="#">'+AItem+'</a>';
    end;
    Result := Result + '</div></li>';
  end
  else
    Result := '<li class="nav-item"><a id="'+FNavBar.Name+C_ITEM+ID.ToString+'" class="nav-link" href="#">'+FText+'</a></li>'
end;
